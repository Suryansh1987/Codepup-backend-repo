...
class IntegrationEngine {
    ...
    generateCleanNaming(componentType, userPrompt) {
        ...
        const promptWords = userPrompt
            ...
            !["page", "component", "create", "make", "build", "add"].includes(word));
    ...
    analyzeNavigationLinks(fileContent, links) {
        return links.map((link) => {
            ...
            if (linkUsageContext.hasScrollBehavior ||
                linkUsageContext.hasScrollIntoView) {
                analysis.type = "in-page-section";
                analysis.isInPageSection = true;
                return analysis;
            }
            if (linkUsageContext.hasRouterContext ||
        ...
        return __awaiter(this, arguments, void 0, function* (generationResult, userPrompt = "") {
            ...
            if (isPageComponent) {
                ...
                if (navigationAnalysis.routeAlreadyExists) {
                    ...
                    if (navigationAnalysis.hasNavigation &&
            ...
            try {
                ...
                if (integrationAnalysis.isPageComponent) {
                    ...
                    if (navAnalysis.routeAlreadyExists) {
                        ...
                        this.streamUpdate(`   🛣️  Routing: ${integrationAnalysis.projectPatterns.appFilePath
                            ? "✅ Required"
                            : "❌ No App File"}`);
                    ...
                    this.streamUpdate(`   📄 Usage Integration: ${integrationAnalysis.usageAnalysis.hasPages
                        ? "✅ Available"
                        : "❌ No Pages Found"}`);
                    if (integrationAnalysis.usageAnalysis.hasPages) {
                        this.streamUpdate(`   🤖 AI Analysis: ${integrationAnalysis.usageAnalysis.aiAnalysis ||
...
${layoutTargets.length > 0
                ? layoutTargets
                    .map((target, index) => `${index + 1}. ${target.fileName} (${target.filePath}) - Layout Component`)
                    .join("\n")
                : "None found"}
...
${pageTargets.length > 0
                ? pageTargets
                    .map((target, index) => `${index + 1}. ${target.fileName} (${target.filePath}) - Page`)
                    .join("\n")
                : "None found"}
...
🔄 Route Already Existed: ${result.integrationResults.routeAlreadyExisted
            ? "YES - Skipped nav updates"
            : "NO - Full integration"}
🧭 Navigation Already Exists: ${result.integrationResults.navigationAlreadyExists
