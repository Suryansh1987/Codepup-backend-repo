...
export class ClaudeFunctionService {
  ...
  async analyzeDesign(
    ...
    if (imageData.length > 0) {
      console.log(
        `📸 Processing ${imageData.length} images for Claude Vision analysis`
      );
      ...
      tools: [
        {
          ...
          description:
            "Save the design analysis and recommendations for the user's business",
          input_schema: {
            type: "object",
            properties: {
              businessType: {
  ...
  private buildVisionPrompt(userPrompt: string, imageCount: number): string {
    let prompt = userPrompt;

    if (imageCount > 0) {
      prompt = `Please analyze the ${imageCount} provided image${
        imageCount > 1 ? "s" : ""
  ...
  private buildVisionSystemPrompt(imageCount: number): string {
    let systemPrompt = `You are a professional business and design consultant with expertise in visual brand analysis.`;

    if (imageCount > 0) {
      systemPrompt += ` You have been provided with ${imageCount} image${
        imageCount > 1 ? "s" : ""
      } to analyze. Use your visual analysis capabilities to:
    ...
    const message = await this.client.messages.create({
      ...
      tools: [
        ...
        },
        {
          ...
          description:
            "User is satisfied with the design, proceed to file generation",
          input_schema: {
            type: "object",
            properties: {
              finalDesign: {
  ...
  async generateDesignFiles(finalDesign: any, userId: string) {
    const generationPrompt = `Create files for: ${JSON.stringify(
      finalDesign,
      null,
      2
    )}`;
    ...
    const message = await this.client.messages.create({
      model: "claude-sonnet-4-0",
      max_tokens: 8000,
      temperature: 0.5, // Very low temperature for predictable behavior
              ...
              files: {
                ...
                properties: {
                  "tailwind.config.ts": {
                    type: "string",
                    description:
                      "Concise Tailwind config with essential colors only",
                  },
                  "/src/index.css": {
                    type: "string",
                    description:
                      "Essential index CSS, as for the vite react project with tailwind",
                  },
                  "design-system.md": {
                    type: "string",
                ...
                required: ["tailwind.config.ts", "/src/index.css"],
              },
              summary: { type: "string" },
            },
  ...
  async generateFileStructurePlan(designChoices: any, userId: string) {
  ...
  async updateDocumentationWithStructure(
    designChoices: any,
    structurePlan: any,
    userId: string
  ) {
              ...
              updatedFiles: {
                ...
                properties: {
                  "design-system.md": {
                    type: "string",
                    description:
                      "Updated design system with file structure info",
                  },
                  "project-structure.md": {
                    type: "string",
                    description:
                      "New file listing all project files and purposes",
                  },
                },
  ...
  async generateBackendFiles(
    ...
    - Include trigger function for new user signup
      ...
      - Create an admin user in both the auth.users table AND the profiles table to ensure proper authentication and authorization.
      ...
      5. Required Tables for E-commerce:
    ...
    - CRITICAL: Create EXACTLY the files listed in the file structure plan above, .env, package.json, etc this is the current package.json {
        ...
        :
            ...
            **MANDATORY: Use this exact AuthContext pattern:**
            ...
            - [ ] white creating button colours take care of the bg also , dont create the buttons of same colours of that of background colurs , while using schadcn buttons alsways give them colours dont assume there colours.
  ...
  async generateFrontendFilesWithProgress(
    designChoices: any,
    fileStructure: any,
    backendFiles: any,
    userId: string,
    ...
    let detectedFiles = new Set<string>();
      ...
      switch (messageStreamEvent.type) {
        ...
        case "content_block_start":
          if (messageStreamEvent.content_block.type === "tool_use") {
            console.log(
              "🛠️ Function being called:",
              messageStreamEvent.content_block.name
            );
        ...
        case "content_block_delta":
          if (messageStreamEvent.delta.type === "input_json_delta") {
            ...
            if (now - lastProgressUpdate > 1500) {
              ...
              const fileMatches = jsonBuffer.match(
                /"([^"]*\.(tsx?|css|json|env|md|ts))"\s*:/g
              );
              if (fileMatches) {
                const currentFiles = fileMatches.map((match) =>
                  match.replace(/"/g, "").replace(":", "").trim()
                );

                const newFiles = currentFiles.filter(
                  (file) => !detectedFiles.has(file)
                );
              ...
              const fileProgress = Math.min(
                90,
                (filesInProgress.length / estimatedTotalFiles) * 85
              );
              const sizeProgress = Math.min(
                10,
                (jsonBuffer.length / 100000) * 10
              ); // Size factor
              const totalProgress = Math.min(
                95,
                fileProgress + sizeProgress + 10
              ); // +10 for initial setup
              ...
              console.log(
                `🔧 Progress: ${totalProgress.toFixed(
                  1
                )}% - ${currentStage} (${Math.floor(
                  jsonBuffer.length / 1000
                )}KB)`
              );
            }
          }
        ...
        case "content_block_stop":
          ...
          if (messageStreamEvent.content_block?.type === "tool_use") {
            ...
            if (toolUse.input?.files) {
              const fileCount = Object.keys(toolUse.input.files).length;
              console.log(
                `📁 Frontend generation completed! Generated ${fileCount} files.`
              );
  ...
  private async handleFunctionCall(message: any, userId: string) {
    ...
    if (message.stop_reason === "tool_use") {
      ...
      if (toolUse) {
        ...
        switch (toolUse.name) {
          ...
          case "generate_design_files":
            ...
            console.log(
              `📁 File count:`,
              Object.keys(toolUse.input.files || {}).length
            );
      ...
      response.message =
        textContent?.text || "I couldn't process that request properly.";
      ...
      if (
        response.message.includes("generate") ||
