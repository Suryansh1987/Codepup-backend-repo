...
export class IntegrationEngine {
  ...
  private generateCleanNaming(
    ...
    const promptWords = userPrompt
      ...
      .filter(
        (word) =>
          word.length > 2 &&
          !["page", "component", "create", "make", "build", "add"].includes(
            word
          )
      );
    ...
    if (
      ...
      !cleanName.toLowerCase().includes(promptWords[0])
    ) {
      const promptBasedName =
        ...
        if (projectFiles.has(pattern)) {
          ...
          if (file?.content) {
            ...
            if (hasNavLinks) {
              ...
              const linkAnalysis = this.analyzeNavigationLinks(
                file.content,
                existingLinks
              );
              const pageRouteCount = linkAnalysis.filter(
                (l) => l.isPageRoute
              ).length;
              const inPageSectionCount = linkAnalysis.filter(
                (l) => l.isInPageSection
              ).length;
              ...
              const hasMatchingRoute =
                existingRoutes.includes(targetRoutePath) ||
                existingRoutes.some(
                  (route) =>
  ...
  private analyzeNavigationLinks(
    ...
    return links.map((link) => {
      ...
      const isPageRoute = pageRoutePatterns.some((pattern) =>
        pattern.test(link)
      );
      ...
      if (isPageRoute) {
        ...
        analysis.displayName = this.extractDisplayNameFromNavigation(
          fileContent,
          link
        );
      ...
      if (
        linkUsageContext.hasScrollBehavior ||
        linkUsageContext.hasScrollIntoView
      ) {
        analysis.type = "in-page-section";
        analysis.isInPageSection = true;
        return analysis;
      ...
      if (
        linkUsageContext.hasRouterContext ||
        linkUsageContext.hasNavigateFunction
      ) {
        analysis.type = "page-route";
        analysis.isPageRoute = true;
        analysis.normalizedPath = this.normalizeRoutePath(link);
  ...
  private async createIntegrationPlan(
    ...
    this.streamUpdate(
      `   Original: ${componentType.name} -> Clean: ${cleanNaming.displayName}`
    );
    ...
    if (isPageComponent) {
      ...
      const navigationAnalysis = await this.analyzeNavigation(
        projectFiles,
        cleanNaming.routePath,
        cleanNaming.displayName
      );

      if (navigationAnalysis.routeAlreadyExists) {
        this.streamUpdate(
          `   ⚠️  Route /${cleanNaming.routePath} already exists in ${navigationAnalysis.matchingNavigationFile}`
        );
        this.streamUpdate(
          `   🎯 Strategy: Create component only, skip navigation updates`
        );
        warnings.push(
          `Route /${cleanNaming.routePath} already exists in navigation - component will be created but navigation won't be modified`
        );

        // Only add App.tsx routing if needed
        if (
          projectPatterns.appFilePath &&
          !existingRoutes.includes(`/${cleanNaming.routePath}`)
        ) {
          integrationFiles.push({
            filePath: projectPatterns.appFilePath,
            exists: true,
            purpose:
              "Add routing for new page (route exists in nav but missing from router)",
            required: true,
            priority: 3,
        ...
        if (
          navigationAnalysis.hasNavigation &&
          !navigationAnalysis.shouldUpdateAppOnly
        ) {
          navigationAnalysis.navigationFiles.forEach((navFile, index) => {
            if (navFile.needsRouteUpdate) {
              integrationFiles.push({
                ...
                required:
                  navFile.type === "header" || navFile.type === "footer",
                priority: 4 + index,
          ...
          this.streamUpdate(
            "   🛑 Header is full (6+ routes) - Skipping navigation updates, App.tsx only"
          );
        }
      }
      ...
      this.streamUpdate(
        "   🎯 COMPONENT Integration: Using AI to find best integration targets..."
      );

      const usageAnalysis = await this.analyzeUsageTargets(
        projectFiles,
        componentType,
        userPrompt
      );
  ...
  async integrateComponent(
    ...
    this.streamUpdate(
      "🔗 STEP 2: Starting Enhanced Integration with Smart Naming..."
    );

    try {
      // 2.1: Create integration plan with proper naming and route detection
      this.streamUpdate(
        "📋 2.1: Creating integration plan with smart naming and route detection..."
      );
      const integrationAnalysis = await this.createIntegrationPlan(
        generationResult,
        userPrompt
      );
      ...
      const typeLabel = integrationAnalysis.isPageComponent
        ? "PAGE"
        : "COMPONENT";

      this.streamUpdate(
        `${typeIcon} Type: ${typeLabel} - ${integrationAnalysis.componentDisplayName}`
      );
      this.streamUpdate(`📋 Integration Strategy:`);
      this.streamUpdate(
        `   Main File: ${integrationAnalysis.mainComponentFile.filePath}`
      );
      this.streamUpdate(
        `   Route Path: /${integrationAnalysis.componentRoutePath}`
      );
      ...
      if (integrationAnalysis.isPageComponent) {
        ...
        if (navAnalysis.routeAlreadyExists) {
          this.streamUpdate(
            `   ⚠️  Route already exists in: ${navAnalysis.matchingNavigationFile}`
          );
          this.streamUpdate(
            `   🎯 Strategy: Component creation only, skip navigation`
          );
        } else {
          this.streamUpdate(
            `   🛣️  Routing: ${
              integrationAnalysis.projectPatterns.appFilePath
                ? "✅ Required"
                : "❌ No App File"
            }`
          );
          this.streamUpdate(
            `   🧭 Navigation: ${
              navAnalysis.hasNavigation ? "✅ Available" : "❌ None"
            }`
          );
        }
        ...
        if (navAnalysis.hasNavigation && !navAnalysis.routeAlreadyExists) {
          ...
          navAnalysis.navigationFiles.forEach((navFile) => {
            const status = navFile.hasMatchingRoute
              ? "❌ Route exists"
              : navFile.needsRouteUpdate
              ? "✅ Will update"
              : "⚠️  Full/Skip";
            this.streamUpdate(
              `      ${navFile.type.toUpperCase()}: ${status} (${
                navFile.pageRouteCount
              }/6 routes)`
            );
          });
        }
        ...
        this.streamUpdate(
          `   📄 Usage Integration: ${
            integrationAnalysis.usageAnalysis.hasPages
              ? "✅ Available"
              : "❌ No Pages Found"
          }`
        );
        ...
        if (integrationAnalysis.usageAnalysis.hasPages) {
          this.streamUpdate(
            `   🤖 AI Analysis: ${
              integrationAnalysis.usageAnalysis.aiAnalysis ||
              "Smart targeting applied"
            }`
          );
      ...
      integrationAnalysis.integrationFiles.forEach((plan) => {
        if (plan.skipReason) {
          this.streamUpdate(
            `   ⏭️  SKIP ${plan.filePath} - ${plan.skipReason}`
          );
          return;
        }
        ...
        if (plan.integrationType === "navigation") {
          icon =
            plan.navigationFileType === "header"
              ? "🔝"
        ...
        this.streamUpdate(
          `   ${status} ${icon} ${plan.filePath} - ${plan.purpose}`
        );
      });
      ...
      this.streamUpdate(
        "✅ STEP 2 Complete: Enhanced Integration with Smart Naming finished!"
      );
      return result;
    } catch (error) {
  ...
  private async executeIntegration(
    ...
    const integrationResults = {
      ...
      navigationAlreadyExists:
        integrationAnalysis.navigationAnalysis.routeAlreadyExists,
    };

    // Filter out skipped files
    const activeFiles = [
      integrationAnalysis.mainComponentFile,
      ...integrationAnalysis.integrationFiles,
    ...
    skippedPlans.forEach((plan) => {
      ...
      this.streamUpdate(
        `   ⏭️  Skipped: ${plan.filePath} - ${plan.skipReason}`
      );
    });
    ...
    const batchIntegrationPrompt = integrationAnalysis.isPageComponent
      ? this.createPageIntegrationPrompt(
          generationResult,
          integrationAnalysis,
          contextFiles as any[]
        )
      : this.createComponentIntegrationPrompt(
          generationResult,
          integrationAnalysis,
          contextFiles as any[]
        );
    ...
    integrationResults.dependenciesResolved =
  ...
  private createPageIntegrationPrompt(
    ...
    const filesToGenerate = [
      ...
      .filter((plan) => !plan.skipReason)
      .map((plan) => {
        const navType = plan.navigationFileType
          ? ` (${plan.navigationFileType.toUpperCase()})`
          : "";
        return `- ${plan.exists ? "MODIFY" : "CREATE"} ${
          plan.filePath
        }${navType} (${plan.purpose})`;
      })
      .join("\n");

    // Smart navigation context based on route analysis
    const navigationContext = integrationAnalysis.navigationAnalysis
      .routeAlreadyExists
      ? `
🚨 ROUTE ALREADY EXISTS - COMPONENT CREATION ONLY:
- Route /${cleanRoutePath} already exists in navigation: ${
          integrationAnalysis.navigationAnalysis.matchingNavigationFile
        }
...
- ${navFile.filePath} (${navFile.type.toUpperCase()}):
  * Page Routes: ${navFile.pageRouteCount}/6 ${
      navFile.canAddMorePages ? "(CAN ADD)" : "(FULL)"
    }
...
${generatedContent.replace(
  new RegExp(`export default ${componentType.name}`, "g"),
...
7. Always give full files that you have created , not add comments like add you exixting code
...
`;
  ...
  private createComponentIntegrationPrompt(
    ...
    const filesToGenerate = [
      ...
      .map(
        (plan) =>
          `- ${plan.exists ? "MODIFY" : "CREATE"} ${plan.filePath} (${
            plan.purpose
          })`
      )
      .join("\n");
...
${integrationAnalysis.usageAnalysis.pageFiles
  .map(
    (pageFile) => `
...
`;
  ...
  private async analyzeUsageTargets(
    ...
    const allTargets = [
      ...
      ...allLayoutComponents.map((l) => ({
        ...l,
        targetType: "layout-component" as const,
      })),
    ];
    ...
    const pageFiles: PageFileInfo[] = aiAnalysis.recommendedPages.map(
      (rec, index) => {
        ...
        return {
          ...
          componentImports: file
            ? this.extractComponentImports(file.content)
            : [],
          aiReason: rec.reason,
        };
      }
  ...
  private async getAIIntegrationAnalysis(
    ...
    const layoutTargets = availableTargets.filter(
      (t) => t.targetType === "layout-component"
    );

    const isNavigationComponent = this.isNavigationComponent(
      componentType,
      userPrompt
    );
...
${
  pageTargets.length > 0
    ? pageTargets
        .map(
          (target, index) =>
            `${index + 1}. ${target.fileName} (${target.filePath}) - Page`
        )
        .join("\n")
    : "None found"
}
...
`;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        const validRecommendations = analysis.recommendedPages.filter(
          (rec: any) =>
  ...
  private isNavigationComponent(
    ...
    if (
      navigationPatterns.some((pattern) => pattern.test(componentType.name))
    ) {
      return true;
    }
    ...
    if (
      navigationPatterns.some((pattern) =>
        pattern.test(componentType.description ?? "")
      )
    ) {
      return true;
    }
  ...
  private getFallbackRecommendations(
    ...
    const priorityPatterns = [
      {
        patterns: [/home/i, /index/i, /dashboard/i],
        isMain: true,
        reason: "Main landing page - good for showcasing new components",
      },
      {
        patterns: [/app/i, /main/i],
        isMain: true,
        reason: "Main application page",
      },
      {
        patterns: [/about/i, /profile/i],
        isMain: false,
        reason: "Content page suitable for component demonstration",
      },
    ];

    for (const pattern of priorityPatterns) {
      const matchingPage = availablePages.find((page) =>
        pattern.patterns.some(
          (p) => p.test(page.fileName) || p.test(page.filePath)
        )
      );

      if (
        matchingPage &&
        !recommendations.some((r) => r.filePath === matchingPage.filePath)
      ) {
  ...
  private hasNavigationLinks(content: string): boolean {
    ...
    const hasNavContent = contentPatterns.some((pattern) =>
  ...
  private extractComponentImports(content: string): string[] {
    ...
    const importMatches =
      content.match(/import\s+(\w+)\s+from\s+['"][^'"]*['"]/g) || [];
  ...
  private parseGeneratedFiles(
    ...
    const filePattern =
      /=== FILE: (.+?) ===\s*```(?:tsx|typescript|jsx|javascript)\s*([\s\S]*?)```/g;
    let match;
    ...
    while ((match = filePattern.exec(responseText)) !== null) {
      ...
      if (content && content.length > 0) {
        ...
        this.streamUpdate(
          `   📄 Parsed: ${filePath} (${content.length} chars)`
        );
      }
    }
    ...
    if (results.size === 0) {
      this.streamUpdate(
        "   ⚠️  Exact format not found, trying alternative parsing..."
      );
      ...
      if (codeBlocks && plans.length > 0) {
        codeBlocks.forEach((block, index) => {
          ...
          if (content && index < plans.length) {
            ...
            this.streamUpdate(
              `   📄 Alt parsed: ${plan.filePath} (${content.length} chars)`
            );
          }
        });
      }
...
🔄 Route Already Existed: ${
      result.integrationResults.routeAlreadyExisted
        ? "YES - Skipped nav updates"
        : "NO - Full integration"
    }
🧭 Navigation Already Exists: ${
      result.integrationResults.navigationAlreadyExists
        ? "YES - Avoided duplicates"
        : "NO - Added new links"
    }
...
${
  result.error
    ? `❌ ERROR: ${result.error}`
    : "✅ Smart Integration completed successfully!"
}
