...
import { pro } from "../defaults/promt";
...
export const conversations = pgTable("conversations", {
  ...
  projectId: integer("project_id")
    ...
    .notNull()
    .unique(), // 🔥 UNIQUE constraint - one conversation per project
  userId: integer("user_id")
    .references(() => users.id, {
      onDelete: "cascade",
    })
...
export const messageSummaries = pgTable(
  "ci_message_summaries",
  {
...
export const ciMessages = pgTable(
  "ci_messages",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    sessionId: text("session_id").notNull(),
    ...
    modificationApproach: varchar("modification_approach", {
      length: 30,
    }).$type<
      | "FULL_FILE"
      | "TARGETED_NODES"
    ...
    stepType: varchar("step_type", { length: 50 }).$type<
      "analysis" | "modification" | "result" | "fallback" | "user_request"
    >(),
    ...
    projectSummaryId: uuid("project_summary_id").references(
      () => projectSummaries.id
    ),
  },
  (table) => ({
    createdAtIdx: index("idx_ci_messages_created_at").on(
    ...
    projectSummaryIdIdx: index("idx_ci_messages_project_summary_id").on(
      table.projectSummaryId
    ),
  })
);

export const conversationStats = pgTable(
  "ci_conversation_stats",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    ...
    lastModificationAt: timestamp("last_modification_at", {
      withTimezone: true,
    }),
...
export const projectSummaries = pgTable(
  ...
  (table) => ({
    createdAtIdx: index("idx_ci_project_summaries_created_at").on(
      table.createdAt
    ),
    sessionIdIdx: index("idx_ci_project_summaries_session_id").on(
      table.sessionId
    ),
    projectIdIdx: index("idx_ci_project_summaries_project_id").on(
      table.projectId
    ),
...
export const sessionModifications = pgTable(
  "ci_session_modifications",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    sessionId: text("session_id").notNull(),
    projectId: integer("project_id").references(() => projects.id, {
...
export const projectSessionsRelations = relations(
  projectSessions,
  ({ one }) => ({
    project: one(projects, {
...
export const projectDeploymentsRelations = relations(
  projectDeployments,
  ({ one }) => ({
    project: one(projects, {
...
export const sessionModificationsRelations = relations(
...
export const conversationStatsRelations = relations(
  conversationStats,
  ({ one }) => ({
    project: one(projects, {
